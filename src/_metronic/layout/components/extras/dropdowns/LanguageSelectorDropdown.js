...
export function LanguageSelectorDropdown() {
    const languageSto = localStorage.getItem('language')
    var languagePage = 'vi';
    if(!languageSto){
      localStorage.setItem("language", 'vi');
      languagePage = 'vi';
    }else{
      languagePage=languageSto;
    }
    let currentLanguage = languages.find((x) => x.lang === languagePage);
  const lang = useLang();
  return (
    <Dropdown drop="down" alignRight>
      ...
      currentLanguage = languages.find((x) => x.lang === language.lang);
